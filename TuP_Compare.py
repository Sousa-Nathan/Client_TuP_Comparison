...
def tup_comparison(wb_dir1, wb_dir2, out_dir):
    ...
    for data in range(len(tup_workbook_1_data)):
        ...
        for item in zip(rows, cols):
            tup_workbook_1_data[data].iloc[item[0], item[1]] = f"New: {tup_workbook_2_data[data].iloc[item[0], item[1]]:.1f} | Delta: {tup_workbook_1_data[data].iloc[item[0], item[1]]:.1f} -> {tup_workbook_2_data[data].iloc[item[0], item[1]]:.1f}"
        ...
        try:
            ...
            with pd.ExcelWriter(out_dir) as writer: # pylint: disable=abstract-class-instantiated
                tup_workbook_1_data[data].iloc[:, :20].to_excel(writer, sheet_name = f"{tup_data_sheets[data]}", index = False)
...
if __name__ == "__main__":
    ...
    tup_workbook_1 = filedialog.askopenfilename(title = "TuP 1", filetypes = (("Excel File", "*.xlsx*"),))
